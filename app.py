...
class User(UserMixin, db.Model):
    ...
    favorite_books = db.relationship(
        "Book", secondary="favorites", back_populates="favorited_by"
    )
...
class Book(db.Model):
    ...
    favorited_by = db.relationship(
        "User", secondary="favorites", back_populates="favorite_books"
    )
...
def api_books():
    try:
        if request.method == "POST":
            ...
            return (
                jsonify({"message": "Book added successfully", "id": new_book.id}),
                201,
            )
        else:
...
def book_details(id):
    ...
    if current_user.is_authenticated:
        is_favorite = (
            Wishlist.query.filter_by(user_id=current_user.id, book_id=id).first()
            is not None
        )
...
def add_edit_book(book_id=None):
    ...
    if request.method == "POST":
        try:
            ...
            app.logger.debug(
                f"Received form data: title={title}, author={author}, publication_year={publication_year}, isbn={isbn}"
            )
            ...
            if book:
                ...
                # Add a new book associated with the logged-in user
                ...
                book = Book(
                    ...
                    cover_url=cover_url,
                    user_id=current_user.id,  # Associate the book with the current user
                )
                db.session.add(book)
            ...
            db.session.commit()
            app.logger.info(
                f"Book {'updated' if book_id else 'added'} successfully with id: {book.id}"
            )
            ...
            db.session.rollback()
            app.logger.error(
                f"Database error {'updating' if book else 'adding'} book: {str(e)}"
            )
            ...
            db.session.rollback()
            app.logger.error(
                f"Unexpected error {'updating' if book else 'adding'} book: {str(e)}"
            )
            flash("An unexpected error occurred. Please try again later.", "error")
            return render_template("add_edit_book.html", book=book), 500

    # Render the form for adding or editing a book
    return render_template("add_edit_book.html", book=book)
...
def add_review(book_id):
    ...
    try:
        ...
        db.session.rollback()
        app.logger.error(
            f"Unexpected error adding review for book_id {book_id}: {str(e)}"
        )
        flash(
            "An unexpected error occurred while adding the review. Please try again later.",
            "error",
        )
        return redirect(url_for("book_details", id=book_id))
